...
Tablelinker コマンドの基本的な用途は、表データファイルに
:ref:`convertor` を適用して変換処理を行い、結果を CSV ファイルに
保存することです。
...
- サンプルデータ
    ...
    ``ma030000.csv`` をダウンロードしてサンプルとして利用します。


ファイルにコンバータを適用する Tablelinker コマンドの文法は次の通りです。

.. code-block:: bash

    $ python -m tablelinker -i 入力ファイル タスクファイル(後述)

タスクファイルは省略可能で、省略した場合は入力ファイルのデータ部分を
そのまま出力します。
...
- 出力ファイル指定
    ...
    また、``--sjis`` オプションを指定するとシフトJIS (CP932) で
    出力できます（デフォルトの文字エンコーディングは `UTF-8` です）。
...
- タスクをコマンドラインで指定
    ...
    ``-p``, ``--params`` オプションでパラメータ（JSON記法）を指定すると、
    タスクファイルを作らずに直接コマンドラインでタスクを指定できます。

    次の例は先頭から三行をタイトル列として結合します。

    .. code-block:: bash

        $ python -m tablelinker -c concat_title -p '{"lines":3,"separator":" "}'\
          -i ma030000.csv
        ,人口,出生数,死亡数,（再掲） 乳児死亡数,新生児 死亡数,自　然 増減数,死産数 総数,自然死産,人工死産,周産期死亡数 総数,22週以後 の死産数,早期新生児 死亡数,婚姻件数,離婚件数
        01 北海道,5188441,29523,65078,59,25,-35555,728,304,424,92,75,17,20904,9070
        02 青森県,1232227,6837,17905,18,15,-11068,145,87,58,32,17,15,4032,1915
        03 岩手県,1203203,6718,17204,8,3,-10486,150,90,60,21,19,2,3918,1679
        04 宮城県,2280203,14480,24632,27,15,-10152,311,141,170,56,41,15,8921,3553
...
- 自動マッピング
    ...
    ``-a`` と共に ``-o <file>`` または ``--output=<file>`` を指定すると、
    変換した結果の CSV データを ``<file>`` に出力します。

    また、``--sjis`` オプションを指定するとシフトJIS (CP932) で
    出力できます（デフォルトの文字エンコーディングは `UTF-8` です）。

    ``-m`` または ``--merge`` を指定すると、変換結果を出力ファイルの
    末尾に追記します。列の並び、文字エンコーディング、
